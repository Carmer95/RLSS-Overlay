 ...
 * @type {WebSocket}
 * Websocket Port: 49322
 * RocketLeagueHost: localhost: 49122
 */
let wsClient;
...
function sendRelayMessage(senderConnectionId, message) {
    ...
    for (let k in connections) {
        ...
        if (connections[k].registeredFunctions.indexOf(
            json["event"]
        ) > -1
    ) {
            setTimeout(() => {
...
function initRocketLeagueWebsocket(rocketLeagueHost) {
    ...
    wsClient.onerror = function (err) {
        err.wb(`Error connecting to Rocket League on host "${rocketLeagueHost}"
